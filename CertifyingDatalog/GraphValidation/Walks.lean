...
section dedup
  ...
  theorem List.drop_until_isWalk_of_isWalk [Hashable A] {G : Graph A} {l : List A} {a : A} (h : l.isWalk G) :
  ...
  theorem List.walk_dedup_isWalk [Hashable A] {G : Graph A} (w : Walk G) : List.isWalk (List.walk_dedup w.1) G := by
