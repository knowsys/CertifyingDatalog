...
namespace Graph
  def toTree_of_acyclic (G : Graph A) (root : {a : A // a ∈ G.vertices}) (acyclic : G.isAcyclic) : Tree A := 
    Tree.node root ((G.successors root).attach.map (fun ⟨node, node_mem⟩ => 
      have _termination : (G.reachableFrom node).card < (G.reachableFrom root).card := by 
        apply Finset.card_lt_card
        apply reachableFromStrictSupersetReachableFromSucc
        apply acyclic
        apply node_mem
      G.toTree_of_acyclic ⟨node, by apply mem_of_is_succ; apply node_mem⟩ acyclic))
  termination_by Finset.card (G.reachableFrom root)
  ...
  def locallyValid_for_kb (G : Graph (GroundAtom τ)) (kb : KnowledgeBase τ) (node : GroundAtom τ) : Prop := 
    (∃ r ∈ kb.prog, ∃ (g : Grounding τ), g.applyRule' r = { head := node, body := G.successors node }) ∨ (G.successors node = [] ∧ kb.db.contains node)
  ...
  def check_local_validity (G : Graph (GroundAtom τ)) (m : SymbolSequenceMap τ) (d : Database τ) (node : GroundAtom τ) : Except String Unit :=
    ...
    if succs.isEmpty 
      then if d.contains node then Except.ok () else checkRuleMatch m ⟨node, succs⟩ 
      else checkRuleMatch m ⟨node, succs⟩
  ...
  lemma check_local_validity_unit_iff_locallyValid (G : Graph (GroundAtom τ)) (kb : KnowledgeBase τ) (node : GroundAtom τ) :
    G.check_local_validity kb.prog.toSymbolSequenceMap kb.db node = Except.ok () ↔ G.locallyValid_for_kb kb node := by 
      have : checkRuleMatch kb.prog.toSymbolSequenceMap ⟨node, G.successors node⟩ = Except.ok () ↔ (∃ r ∈ kb.prog, ∃ (g : Grounding τ), g.applyRule' r = { head := node, body := G.successors node }) := by 
